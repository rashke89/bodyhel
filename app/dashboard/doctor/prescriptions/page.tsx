...
export default function DoctorPrescriptionsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'active' | 'completed' | 'cancelled' | 'expired'>(
    'all'
  );
  ...
  const filteredPrescriptions = useMemo(() => {
    ...
    return prescriptions.filter((presc) => {
      ...
      return patientName.includes(term) || patientId.includes(term) || eId.includes(term);
    });
    ...
    try {
      const payload: any = {
        ...
        medications: formData.medications.map((med) => ({
          ...
          dosage: med.dosageAmount || med.dosageUnit ? { amount: med.dosageAmount, unit: med.dosageUnit } : undefined,
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-600 mt-1">Pregled recepata koje ste izdali</p>
          </div>
          <button
            ...
            {filteredPrescriptions.map((presc) => (
              ...
              >
                  ...
                  <div>
                    ...
                    {presc.expiryDate && (
                      <p className="text-sm text-gray-500">
                        Ističe: {new Date(presc.expiryDate).toLocaleDateString('sr-RS')}
                      </p>
                    )}
                    {presc.diagnosis && (
                ...
                {presc.ePrescriptionId && (
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <p className="text-xs text-gray-500">e-Recept ID: {presc.ePrescriptionId}</p>
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center">
              <h2 className="text-2xl font-semibold text-[#2C6975]">Kreiraj recept</h2>
              <button
                onClick={() => {
                  setCreateModalOpen(false);
                  resetForm();
            ...
            <form onSubmit={handleCreatePrescription} className="p-6 space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Pacijent *</label>
                <select
                  required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Dijagnoza (šifra)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Dijagnoza (opis)</label>
                  <input
                    type="text"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-lg font-semibold text-gray-900">Lekovi *</h3>
                  <button
                    type="button"
                  ...
                  {formData.medications.map((med, idx) => (
                    <div key={idx} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-4">
                        <p className="font-medium text-gray-900">Lek {idx + 1}</p>
                        <button
                          type="button"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">Generički naziv</label>
                          <input
                            type="text"
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) => handleMedicationChange(idx, 'dosageUnit', e.target.value)}
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">Učestalost *</label>
                          <input
                            type="text"
                            required
                            value={med.frequency}
                        ...
                        <div className="md:col-span-2">
                          <label className="block text-sm font-medium text-gray-700 mb-2">Uputstvo</label>
                          <input
                            type="text"
                        ...
                        <div className="md:col-span-2">
                          <label className="inline-flex items-center space-x-2">
                            <input
                              ...
                              onChange={(e) =>
                                handleMedicationChange(idx, 'substitutionsAllowed', e.target.checked)
                              }
                              className="rounded border-gray-300 text-[#6BB2A0] focus:ring-[#6BB2A0]"
                            />
                            <span className="text-sm text-gray-700">Dozvoljena zamena</span>
                          </label>
                        </div>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Datum isteka</label>
                  <input
                    type="date"
