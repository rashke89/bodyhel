...
export default function DoctorDashboard() {
  ...
  const [followUpModal, setFollowUpModal] = useState<{ open: boolean; appointmentId: string | null }>({
    open: false,
    appointmentId: null,
  });
  const [followUpData, setFollowUpData] = useState({ date: '', startTime: '09:00', reason: '' });
  ...
  const fetchDashboard = async () => {
    try {
      ...
      const completed = appointmentsRes.appointments?.filter((a: any) => a.status === 'completed').length || 0;
      const pending = appointmentsRes.appointments?.filter((a: any) => a.status === 'scheduled' || a.status === 'confirmed').length || 0;
  ...
  const handleFollowUp = async () => {
    if (!followUpModal.appointmentId || !followUpData.date || !followUpData.startTime) return;
    setFollowUpLoading(true);
    try {
      await api.scheduleFollowUp(followUpModal.appointmentId, followUpData);
      setFollowUpModal({ open: false, appointmentId: null });
      setFollowUpData({ date: '', startTime: '09:00', reason: '' });
      fetchDashboard();
    } catch (error: any) {
      ...
      <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-[#2C6975]">{stats?.totalToday || 0}</p>
              </div>
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200 flex justify-between items-center">
            <h2 className="text-xl font-semibold text-[#2C6975]">Današnji pregledi</h2>
            <Link
          ...
          <div className="p-6">
            {todayAppointments.length > 0 ? (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col space-y-2">
                        ...
                        {apt.status === 'completed' && (
                          <button
                            onClick={() => setFollowUpModal({ open: true, appointmentId: apt._id })}
              ...
              <p className="text-gray-500 text-center py-8">Nemate pregleda za danas</p>
            )}
          </div>
        </div>
      </div>
      ...
      {followUpModal.open && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-[#2C6975] mb-4">Zakaži kontrolni pregled</h3>
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFollowUpData({ ...followUpData, startTime: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#6BB2A0]"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Razlog</label>
                <input
                  type="text"
            ...
            <div className="flex justify-end space-x-3 mt-6">
              ...
              <button
                ...
                disabled={followUpLoading || !followUpData.date}
